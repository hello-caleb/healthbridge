...
import { Video, Mic, Globe, Info, Power, Mic as MicIcon, MicOff, ExternalLink, RefreshCw } from 'lucide-react';
    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-slate-50 text-slate-900 p-4 font-sans">
                {/* Reconnection Banner */}
                {isReconnecting && (
                    <div className="fixed top-0 left-0 right-0 z-40 bg-amber-500 text-white py-3 px-4 text-center font-semibold flex items-center justify-center gap-2 animate-pulse">
                ...
                {/* Header */}
                <header className={`mb-4 flex flex-col gap-4 border-b border-slate-200 pb-4 ${isReconnecting ? 'mt-12' : ''}`}>
                ...
                <main className="grid h-[calc(100vh-100px)] grid-cols-1 gap-6 lg:grid-cols-12">
                    ...
                    <section className="relative col-span-1 flex flex-col gap-6 lg:col-span-9 lg:h-full">
                        ...
                        <div className="min-h-[200px] shrink-0 rounded-3xl bg-white p-6 border-2 border-slate-200 shadow-md flex flex-col">
                            ...
                            <div ref={scrollRef} className="flex-1 overflow-y-auto max-h-[250px] p-2 space-y-3" aria-live="polite">
                                {speakerSegments.length === 0 ? (
                                    <p className="text-slate-300 italic text-center mt-4">Captions will appear here...</p>
                                ) : (
                                    speakerSegments.map((segment, index) => {
                                        const styles = getSpeakerStyles(segment.speaker);
                                        return (
                                            <div
                                                key={index}
                                                className={`rounded-xl p-4 ${styles.container} animate-in slide-in-from-bottom fade-in duration-200`}
                                            >
                                                ...
                                                <p className={`leading-relaxed text-slate-800 font-semibold whitespace-pre-wrap font-sans ${captionSize === 'sm' ? 'text-base lg:text-lg' : captionSize === 'md' ? 'text-xl lg:text-2xl' : 'text-2xl lg:text-4xl'}`}>
                                                    {segment.text}
                                                </p>
                                            </div>
                                        );
...
// Export Dashboard wrapped in ErrorBoundary for production resilience
